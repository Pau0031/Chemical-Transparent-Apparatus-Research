...
def get_slice_bboxes(
    ...
    while y_max < image_height:
        ...
        while x_max < image_width:
            ...
            x_min = x_max - x_overlap  # 根据重合区域，重新确定横向x坐标
        y_min = y_max - y_overlap  # 根据重合区域，重新确定纵向y坐标
    return slice_bboxes  # 网格坐标计算完毕
...
def slice_image(
    ...
    # slice_bboxes:list[[x_min, y_min, x_max, y_max]]
    ...
    # init images and annotations lists, this is an operation object
    ...
    for slice_bbox in slice_bboxes:
        ...
        # process annotations if coco_annotations is given, 加工标注信息
        ...
        # create coco image
        slice_width = slice_bbox[2] - slice_bbox[0]  # 切片图像的宽度
        slice_height = slice_bbox[3] - slice_bbox[1]  # 切片图像的高度
        ...
        if coco_annotation_list:
            for coco_annotation in sliced_coco_annotation_list:
                coco_image.add_annotation(coco_annotation)  # 添加处理后标注信息

        # create sliced image and append to sliced_image_result，包含图像矩阵、标注信息（宽和高）、起始坐标
        sliced_image = SlicedImage(
            image=image_pil_slice, coco_image=coco_image, starting_pixel=[slice_bbox[0], slice_bbox[1]]
        )
        sliced_image_result.add_sliced_image(sliced_image)  # list append the object
    ...
    return sliced_image_result  # 完成图像的切分和标注信息的切分
