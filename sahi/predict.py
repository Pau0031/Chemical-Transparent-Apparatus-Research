...
def get_prediction(
    ...
    # process prediction
    time_start = time.time()
    # works only with 1 batch, convert data type from tensor to ndarray
...
def get_sliced_prediction(
    ...
    # init match postprocess instance, default:"GREEDYNMM"
    ...
    for group_ind in range(num_group):
        ...
        for object_prediction in prediction_result.object_prediction_list:
            if object_prediction:  # if not empty
                object_prediction_list.append(object_prediction.get_shifted_object_prediction())  # 还原坐标
...
def predict(
    ...
    for ind, image_path in enumerate(
        ...
        if source_is_video:
            ...
            relative_filepath = relative_filepath[1:] if relative_filepath[0] == os.sep else relative_filepath  # 修正relative_filepath的路径
